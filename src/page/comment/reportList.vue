  ...
  <div class="videoList">
    <el-card>
      <el-table
        ...
        <el-table-column label="视频/评论" width="420" align="left">
          <template slot-scope="scope">
            <div style="cursor: pointer" @click="toEdit(scope.row.media_info.media_id)">
              <div class="videotext"><span class="comment_tips">配文</span>{{ scope.row.media_info.media_desc }}</div>
              <div class="yinyong"><p v-if="scope.row.comment_info.quote && scope.row.comment_info.quote.content"><span class="comment_tips">引用</span>{{scope.row.comment_info.quote.content}}</p></div>
              <div class="comtext"><span class="comment_tips">评论</span>{{ scope.row.comment_info.content }}</div>
        ...
        <el-table-column prop="fav_cnt" label="发布用户" width="160" align="center">
          <template slot-scope="scope">{{ scope.row.comment_info.user_info.username }}</template>
        </el-table-column>
        ...
        <el-table-column prop="show_status" label="操作" align="center">
          <template slot-scope="scope">
        ...
        <el-table-column sortable label="状态" width="70" align="center">
          <template slot-scope="scope">{{ scope.row.comment_info.status_desc || '未审核'}}</template>
        </el-table-column>
      </el-table>
      <div class="stock_dom">
        <el-button round @click="toggleSelection(reportList)">反选</el-button>
        <el-button round @click="toggleSelection()">清除</el-button>
        <el-button round type="danger" @click="deleteInfo(scope.row.id)">删除</el-button>
        <el-pagination
...
export default {
  ...
  methods: {
    getData () {
      this.loading = true
      this.$axios.get(`/admin/comment/commentReportList?page=${this.currentPage}&size=20`).then((res) => {
        console.log(res)
        if (res.data.code === 0) {
          this.reportList = res.data.data.comment_list
